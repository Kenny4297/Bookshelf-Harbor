...
const ShoppingCart = () => {
    ...
    const removeFromCart = async (bookId) => {
        try {
            await axios.post(`/api/user/${userId}/cart/remove`, { bookId });
    ...
    if (isLoading) {
        ...
        return (
            <section className="shopping-cart-container" aria-label="Shopping cart content">
                <h2 className="shopping-cart-h2">Shopping Cart</h2>
                {cartItems.length > 0 ? (
                    <section>
                        <section className="shopping-cart-only-book-container" aria-label="List of books in cart">
                            {cartItems.map((book, index) => (
                                <div className="shopping-cart-book" key={index} aria-label={`Book title: ${book.title}`}>
                                    ...
                                    <p className="shopping-cart-price">
                                        Price: <span className="shopping-cart-rendered-price">${book.price}</span>
                                    </p>

                                    <button className="shopping-cart-remove-button" aria-label="Remove book from cart" onClick={() => removeFromCart(book._id)}>
                                        Remove from Cart
                                    </button>
                                </div>
                            ))}
                        ...
                        <section className="shopping-cart-totals-container" aria-label="Cart totals">
                            <section className="shopping-cart-math-container">
                                <p className="standard">
                                    Pre-tax total: <span className="generated">${totals.preTaxTotal}</span>
                                </p>
                                <p>
                                    Sales tax (6%): <span className="generated">${totals.salesTax}</span>
                                </p>
                                <p>
                                    Shipping Cost: <span className="generated">${totals.shippingCost}</span>
                                </p>
                            </section>
                            <hr />
                            <p className="standard">
                                Total with tax and shipping: <span className="generated shopping-cart-total">${totals.totalWithTaxAndShipping}</span>
                            </p>

                            <button className="shopping-cart-clear-cart" onClick={clearCart}>
                                Clear Cart
                            </button>
                            <Link className="shopping-cart-clear-cart" to={`/checkout/${userId}`} aria-label="Checkout">
                                Checkout
                            </Link>
                        </section>
                    </section>
                    ...
                    >
                        ...
                        <Link className="back-to-home" to="/" aria-label="Back to home">
                            Back to home
                        </Link>
                    </div>
                )}
